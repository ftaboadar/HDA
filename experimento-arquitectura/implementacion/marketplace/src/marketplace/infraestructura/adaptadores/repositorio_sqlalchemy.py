...
from marketplace.dominio.entidades import Marketplace, Diagnostico, Cotizacion
import uuid
...
class RepositorioMarketplaceSQLAlchemy(RepositorioMarketplace):
    ...
    def obtener_por_solicitud_id(self, solicitud_id: uuid.UUID) -> Marketplace:
        ...
        model = session.query(MarketplaceModel).filter_by(solicitud_id=str(solicitud_id)).first()
        session.close()
        if model:
            mkp = Marketplace(id=uuid.UUID(model.id), solicitud_id=uuid.UUID(model.solicitud_id))
            if model.proveedor_seleccionado_id:
                mkp.proveedor_seleccionado_id = uuid.UUID(model.proveedor_seleccionado_id)
            return mkp
        return Marketplace(solicitud_id=solicitud_id)
        
    def guardar(self, marketplace: Marketplace):
        session = self.Session()
        model = session.query(MarketplaceModel).filter_by(solicitud_id=str(marketplace.solicitud_id)).first()
        if not model:
            model = MarketplaceModel(id=str(marketplace.id), solicitud_id=str(marketplace.solicitud_id))
            session.add(model)
