"""Worker estándar (CONVENCIONES §5) — PLANTILLA de la Entrega 5.
...
`crear_app_worker` arranca todos los consumidores del servicio en hilos y expone
`GET /salud`, que responde 200 solo si todos siguen vivos (503 si alguno murió,
para que Cloud Run reinicie la instancia).

Uso en `app/worker/main.py` de cada servicio:

    app = crear_app_worker(lambda: [ConsumidorPulsar(...), ...])

y se corre con `uvicorn app.worker.main:app --port 8080` (o
`python -m app.worker.main`). La fábrica se llama en el startup, no al importar,
para que importar el módulo no conecte a Pulsar."""
...
def crear_app_worker(
    ...
    app = FastAPI(title="Worker de consumidores Pulsar", lifespan=ciclo_de_vida)
