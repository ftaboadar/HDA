...
from app.orquestacion_partner.domain.entidades.value_objects import ReglaDeAprobacion, RedPermitida, MontoMaximo
from app.orquestacion_partner.infrastructure.messaging.publisher import publicar_decision_partner
...
def consumir_comandos():
    ...
    while True:
        try:
            ...
            log_evento("mensaje_recibido", props.get("correlation_id", "N/A"), "WORKER", {"id_evento": props.get("id_evento")})
            
            if data.tipo_comando == "SolicitarAprobacionNovedad":
                partner = Partner(
                    id=data.partner_id,
                    nombre="Aseguradora Alfa",
                    regla=ReglaDeAprobacion(requiere_aprobacion_manual=True, umbral_monto=100.0),
                    red=RedPermitida(["prov1", "prov2"]),
                    monto=MontoMaximo(1000.0, "COP")
                )
                
                decision = partner.evaluar_novedad(data.trabajo_id, data.novedad_id, data.impacto_monto)
