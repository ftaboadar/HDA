...
class Partner(AggregateRoot):
    ...
    def evaluar_novedad(
        self, trabajo_id: str, novedad_id: str, impacto_monto: float
    ) -> str:
        if (
            not self.regla_de_aprobacion.requiere_aprobacion_manual
            or impacto_monto <= self.regla_de_aprobacion.umbral_monto
        ):
