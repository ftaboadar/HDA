...
| **A26 Tecnologías de BD y Optimización de Costos** | **Optimización de costos de infraestructura** sin degradar **escalabilidad/disponibilidad**. Dentro de la topología descentralizada (BD lógica por servicio, sincronizando datos vía eventos en Pulsar), se elige **Postgres (Cloud SQL)** para todos. Permite transacciones ACID (Saga Log + estado) y agrupar esquemas aislando por BD lógica abaratando costos. Se usa **CRUD** para datos de estado (Trabajo, Pago) y **Event Sourcing** solo donde el historial es core (Reputación). | El aislamiento físico absoluto (instancias Cloud SQL dedicadas por microservicio), lo cual aumenta el radio de impacto de una caída de base de datos a cambio de viabilizar el presupuesto. | **NoSQL (Mongo)**: se descarta por añadir costo operativo sin ganancia (los datos son altamente relacionales). **Serverless Nativo (Firestore/Spanner)**: descartado por riesgo de sobrecosto impredecible en picos agresivos (ESC-01) y *vendor lock-in*. **Caché (Redis)**: descartado por ahora para ahorrar costos de infraestructura; los picos de lectura se soportan escalando Cloud Run sin la complejidad de invalidar cachés. **ES en todo**: complejidad sin retorno. |
...
| Respuestas (`FranjaReservada`, `PagoRetenido`, `PagoRetencionFallida`, …) | Evento de integración **delgado** (ids + resultado + motivo) | El coordinador ya tiene el estado de la saga; solo necesita el resultado del paso |
