...
| # | Decisión |
|---
| A29: Despliegue Multi-Región y límite de cuotas | Evitar el bloqueo por cuotas (`RESOURCE_EXHAUSTED` de 20 vCPUs por región) durante la inyección del pico de carga (ESC-01). (Eficiencia en costos, Escalabilidad) | Mayor latencia de red entre servicios interregionales y complejidad en los scripts de despliegue de Terraform. | Desplegar todo en la misma región con auto-scaling alto (descartado porque agota la cuota gratuita/estudiantil y bloquea el experimento). ||---
| A29: Despliegue Multi-Región y límite de cuotas | Evitar el bloqueo por cuotas (`RESOURCE_EXHAUSTED` de 20 vCPUs por región) durante la inyección del pico de carga (ESC-01). (Eficiencia en costos, Escalabilidad) | Mayor latencia de red entre servicios interregionales y complejidad en los scripts de despliegue de Terraform. | Desplegar todo en la misma región con auto-scaling alto (descartado porque agota la cuota gratuita/estudiantil y bloquea el experimento). ||
...
| A29 | **Despliegue Multi-Región y límite de instancias en Cloud Run**. Se limitará `max_instances` fuertemente en servicios secundarios y se distribuirán los críticos en distintas regiones (p. ej., Pagos en `southamerica-east1`, Siniestros en `us-central1`, GT en `us-east1`) para evitar agotar la cuota de 20 vCPUs por región durante el pico de carga de k6. |
